...
class falkonconnectViewSet(BaseConnectorViewSet):
    PASSWORD = settings.FALKONCONNECT_PASSWORD
    USERNAME = settings.FALKONCONNECT_USERNAME
    BASE_URL = "https://api.falkonds.com"
    AUTH_TYPE = "basic"
    IDENTIFIER = "FALKONCONNECT"

    @action(detail=False, methods=["get"], url_path="test/search")
    def test_search(self, request, *args, **kwargs):
        params = request.query_params
        payload = request.data
        data = self._api_call(
            "/test/search", request_type="get", payload=payload, params=params
        )
